...
def song_showLike():
    ...
    btn_like = db.users_songs.find_one({'username': username_receive, "title": title_receive, 'artist': artist_receive},
                                       {'_id': False})
...
def like_btn():
    ...
    if is_weatherLike_receive == "true":
        ...
        db.songs.update_one({'title': title_receive, 'artist': artist_receive},
                            {'$set': {weatherBtn_receive: minus_like}})
        ...
        if song == None:
            ...
            db.songs.update_one({'title': title_receive, 'artist': artist_receive},
                                {'$set': {weatherBtn_receive: plus_like}})
    ...
    users_songs = db.users_songs.find_one(
        {'username': username_receive, 'title': title_receive, 'artist': artist_receive})

    # users_songs db 초기화
    users_songs_doc = {'username': username_receive, 'title': title_receive, 'artist': artist_receive, 'Sunny': False,
                       'Cloudy': False, 'Rainy': False, 'Snowy': False, }
    ...
    # 사용자가 곡에 좋아요를 처음 누른 게 아닐때
        ...
        if users_songs[weatherBtn_receive] == False:
            db.users_songs.update_one({'username': username_receive, 'title': title_receive, 'artist': artist_receive},
                                      {'$set': {weatherBtn_receive: True}})

        # 사용자가 좋아요를 취소했을 때 (현재 버튼이 좋아요를 누른 상태일 때)
        else:
            db.users_songs.update_one({'username': username_receive, 'title': title_receive, 'artist': artist_receive},
                                      {'$set': {weatherBtn_receive: False}})
